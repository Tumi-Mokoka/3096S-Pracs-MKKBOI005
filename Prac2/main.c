...
/* USER CODE BEGIN PV
 *
 * MKKBOI005 - Tumi (Boitumelo) Mokoka
 * 29/08/2023
...
static uint16_t mem_addresses [] = {0x0000, 0x0001, 0x0002, 0x0003, 0x0004, 0x0005};
static uint8_t curr_address_index = 0;
#define SW0 0b01
static uint8_t TimerHalved = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  {
	  uint8_t patt = patterns[i];
	  //writing each value into memory starting at 0x0000
	  write_to_address(mem_addresses[i], patterns[i]);
  }
  ...
  while (1)
  {
	...
	// PA0 = SW0 --> using TIM16's ARR value
	  if ((GPIOA->IDR & SW0) == 0)
		  if (TimerHalved == 0)
		  {
			  //makes timer half-second delay
			  TimerHalved = 1;
			  TIM16->ARR = 500-1;
		  }
		  else
		  {
			  //returns timer back to 1s delay
			  TimerHalved = 0;
			  TIM16->ARR = 1000-1;
		  }
...
void TIM16_IRQHandler(void)
{
	...
	curr_address_index++;
	//overflow situation starts at the beginning
	if (curr_address_index >= 6){curr_address_index=0;}

	// reads an LED pattern from memory
	uint8_t LED_Pattern = read_from_address(mem_addresses[curr_address_index]);

	//TASK 5: Fail safe if pattern is not read back correctly
	if (LED_Pattern == patterns[curr_address_index])
	{
