...
/**
  ...
  ******************************************************************************
 	 ...
 	 @Date: 28/09/2023
 	 @Assignment : EEE3096S Practical 3
...
int main(void)
{
  ...
  while (1)
  {
	...
	pollADC(); // read ADC value

	//convert ADC value to string
	//--> https://stackoverflow.com/questions/20222391/how-can-i-convert-a-uint32-t-to-a-char-type
	char adc_str[8];
...
void EXTI0_1_IRQHandler(void)
{
	...
	if (delay_t == 500)
	{
		delay_t = 1000; //1Hz delay time
	}
	else
	{
		delay_t = 500; //switch back to 2Hz delay
	}
...
uint32_t pollADC(void){
  // TODO: Complete function body to get ADC val
	HAL_ADC_Start_IT(&hadc); //Starts ADC conversions
	ADC1_COMP_IRQHandler();
	HAL_ADC_Stop_IT(&hadc); //Stops ADC conversions
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR val using an appropriate equation
	...
	//return the percent of the full ARR range
	uint32_t val = (uint32_t)((adc_val * 47999) / 4095);
