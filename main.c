...
/*
 * @author: Tumi Mokoka (MKKBOI005)
 * @date: 19/10/2023
 *
 *EEE3095S Practical 4 submission
 * */
...
uint32_t saw_LUT[NS] = {
		...
		952, 960, 968, 976, 984, 992, 1000, 1008, 1016

};

/*https://daycounter.com/Calculators/Triangle-Wave-Generator-Calculator2.phtml*/
...
// Write LUT TO TIM3->CCR3 to modify PWM duty cycle
...
void EXTI0_1_IRQHandler(void)
{
	...
	if(currTimeStamp - lastButtonPressTime > 100)
	{

		lastButtonPressTime = currTimeStamp;

		// TODO: --> if debouncing code allows it

			/*
			 * 1. Disable DMA transfer and abort IT
			 * 2. then start DMA in IT mode with new LUT
			 * 3. re-enable transfer
			 */
			...
			switch (curr_LUT)
			{
			case 1: //currently using the sin wave
			...
			case 2: //currently using the sawtooth format
			...
			case 3: //currently using the triangle waveform
				HAL_DMA_Start_IT (&hdma_tim2_ch1, (uint32_t)Sin_LUT, (uint32_t)DestAddress, NS);
				lcd_command(CLEAR);
				lcd_putstring("Sine");
				curr_LUT = 1;
